...
def lbs(betas, pose, v_template, shapedirs, posedirs, J_regressor, parents,
        lbs_weights, pose2rot=True, num_joints=23, dtype=torch.float32,batch_size=None):
    ...
    if pose2rot:
        rot_mats = batch_rodrigues(
            pose.contiguous().view(-1, 3), dtype=dtype).view([batch_size, -1, 3, 3])
...
def batch_rigid_transform(rot_mats, joints, parents, dtype=torch.float32):
    ...
    joints_homogen = torch.cat([joints,
                                torch.zeros([batch_size, num_joints, 1, 1],
                                            dtype=dtype, device=device)],
                               dim=2)
